...
class Allresult extends CI_Controller {
	...
	{	
		...
		if(!empty($resultDate)):
			$whereCon['where']['result_date'] = (int)strtotime($resultDate);
		endif;
       ...
       if($this->uri->segment(getUrlSegment())):
		...
		$this->layouts->set_title('All Result | UWINN');
		$this->layouts->admin_view('uwin/allresult/index',array(),$data);
	}	// END OF FUNCTION
	...
	{		
		...
		$whereCon['where']['enable_raffle_ticket'] = array('$ne'=> 'Enable' );
		$shortField 	    = array('_id'=> -1 );
		...
		if($editId):
			...
			$data['EDITDATA'] =	$this->common_model->getDataByParticularField('uw_draw_result','_id',new MongoDB\BSON\ObjectId($editId));
		else:
			$this->admin_model->authCheck('view_data');
		endif;
		...
		if($this->input->post('SaveChanges')):
			...
			if($this->form_validation->run() && $error == 'NO'):
				...
				$param['draw_result_time'] = stripslashes($productetails[2]);

				$whereCondition['where'] = $param;
				$getResultDraw = $this->common_model->getData('single','uw_draw_result' ,$whereCondition);
				...
				if($this->input->post('CurrentDataID') ==''):
					...
					$param['status']	    = 'A';
					$param['result_id']		= (int)$this->common_model->getNextSequence('uw_draw_result');
					$param['creation_ip']	= currentIp();
					$param['creation_date']	= (int)$this->timezone->utc_time();//currentDateTime();
					$param['created_by']	= (int)$this->session->userdata('UW_ADMIN_ID');
					$alastInsertId			= $this->common_model->addData('uw_draw_result',$param);
					...
					$this->common_model->editData('uw_draw_result',$param,'_id',new MongoDB\BSON\ObjectId($resultId));
					$this->session->set_flashdata('alert_success',lang('updatesuccess'));
				endif;
				redirect(correctLink('ALLRESULTSDATA',getCurrentControllerPath('index')));
			endif;
		endif;
		$this->layouts->set_title('Add Result | UWINN');
		$this->layouts->admin_view('uwin/allresult/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	public function viewdata($editId='')
	{
		...
		$data['EDITDATA'] = $this->common_model->getData('multiple','uw_draw_result',$whereCon);
		// echo "<pre>";print_r($data['EDITDATA']); die();

		$baseUrl = getCurrentControllerPath('index');
		$this->session->set_userdata('ALLRESULTSDATA',currentFullUrl());
		
		$this->layouts->set_title('View Result Details | UWINN');
		$this->layouts->admin_view('uwin/allresult/viewdetails',array(),$data);
	}
	/***********************************************************************
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->editData('uw_draw_result',$param,'_id',new MongoDB\BSON\ObjectId($changeStatusId));
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->deleteData('uw_draw_result','_id',new MongoDB\BSON\ObjectId($deleteId));
	...
	************************************************************************/
	function bulkdeletedata()
	{
		...
		if(!empty($resultDates)):
			...
			$this->mongo_db->delete_all('uw_draw_result');
	...
	************************************************************************/	
	public function settings($editId='')
	{
		...
		if($editId):
			$this->admin_model->authCheck('edit_data');
			$data['EDITDATA'] = $this->common_model->getDataByParticularField('uw_draw_result_settings','_id',new MongoDB\BSON\ObjectId($editId));
		else:
			$data['EDITDATA'] = $this->common_model->getData('single','uw_draw_result_settings');
			$this->admin_model->authCheck('view_data');
		endif;
		...
		if($this->input->post('SaveChanges')):
			...
			if($this->form_validation->run() && $error == 'NO'):
				...
				if($this->input->post('CurrentDataID') == ''):
					$param['status'] = 'A';
					$param['result_id'] = (int)$this->common_model->getNextSequence('uw_draw_result_settings');
					$param['creation_ip'] = currentIp();
					$param['creation_date'] = (int)$this->timezone->utc_time();
					$param['created_by'] = (int)$this->session->userdata('UW_ADMIN_ID');
					$this->common_model->addData('uw_draw_result_settings',$param);
					$this->session->set_flashdata('alert_success',lang('addsuccess'));
				else:
					$resultId = $this->input->post('CurrentDataID');
					$param['update_ip'] = currentIp();
					...
					$param['updated_by'] = (int)$this->session->userdata('UW_ADMIN_ID');
					$this->common_model->editData('uw_draw_result_settings',$param,'_id',new MongoDB\BSON\ObjectId($resultId));
					$this->session->set_flashdata('alert_success',lang('updatesuccess'));
				endif;
				redirect(correctLink('ALLRESULTSDATA',getCurrentControllerPath('index')));
			endif;
		...
		$this->layouts->set_title('Result Settings | UWINN');
		$this->layouts->admin_view('uwin/allresult/settings',array(),$data);
	}
