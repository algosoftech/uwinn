...
class Allapiuser extends CI_Controller {
	...
	{	
		...
		if($this->input->get('searchField') && $this->input->get('searchValue')):
			...
			$whereCon['where']		 		= 	array('users_type'=> 'Api User');
			$data['searchField'] 			= 	'';
			$data['searchValue'] 			= 	'';	
		endif;

		$this->session->set_userdata('ALLAPIUSERSDATA',currentFullUrl());
       ...
       if($this->uri->segment(getUrlSegment())):
		...
		$this->layouts->admin_view('users/allapiuser/index',array(),$data);
	}	// END OF FUNCTION
	...
	{		
		...
		// echo '<pre>';print_r($_POST);die;
			...
			$this->form_validation->set_rules('store_name', 'Store Name', 'trim|is_unique[uw_users.store_name]');
			...
			if($this->form_validation->run() && $error == 'NO'): 
				...
				$param['whitelisted_ips']		 = $this->input->post('whitelisted_ips');
				$param['store_name']		 	 = $this->input->post('store_name');
			    $param['time_zone'] 			 = $this->input->post('time_zone');
				// $param['decode_api_key']		 = 	$this->admin_model->decryptsPassword($param['api_key']); 
				// $param['commission_percentage']	 = 	$this->input->post('commission_percentage');
				//End

				if($this->input->post('CurrentDataID') ==''):
					$param['app_name']				 = 	$this->input->post('app_name');
					$param['api_key']		         = 	$this->admin_model->encriptPassword($this->input->post('api_key')); 
					$param['totalArabianPoints']	 = 	(float)$this->input->post('totalArabianPoints');
				    $param['availableArabianPoints'] = 	(float)$this->input->post('availableArabianPoints');
					$param['referral_code']	         =	strtoupper(uniqid(16));
					$param['users_id']				 =	(int)$this->common_model->getNextSequence('uw_users');
					$param['users_seq_id']			 =	$this->common_model->getNextIdSequence('users_seq_id','Api User');
		...
		$this->layouts->admin_view('users/allapiuser/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	************************************************************************/
	...
	{  
		...
		redirect(correctLink('ALLAPIUSERSDATA',getCurrentControllerPath('index')));
	}
