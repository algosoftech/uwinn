...
class Allrechargecoupons extends CI_Controller {
	...
	{	
		...
		if($this->input->get('fromDate')):
			...
			$whereCon['where_gte'] 			= 	array(array("redeemed_date",strtotime($this->input->get('fromDate'))));
		endif;

		if($this->input->get('toDate')):
			$data['toDate'] 				=   date('Y-m-d H:i', strtotime($this->input->get('toDate')));  //2023-03-16 15:13
			$whereCon['where_lte'] 			= 	array(array("redeemed_date",  strtotime($this->input->get('toDate'))));
		endif;
	...
	************************************************************************/
	...
	{  
		...
		$totalRows    = $this->common_model->getData('count','uw_coupon_code_only',$wcon);
		$itemsPerPage = 5000;
		// ---------------------------------------------
