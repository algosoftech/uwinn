                            ...
                            <form id="currentPageForm" name="currentPageForm" class="form-auth-small" method="post" action="" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group-inner col-lg-4 col-md-4 col-sm-4 col-xs-12 <?php if(form_error('user')): ?>error<?php endif; ?>">
                                        <label class="rechare_for_label">Mobile No.<span class="required">*</span></label>
                                    ...
                                    <div class="form-group-inner col-lg-6 col-md-6 col-sm-6 col-xs-12 <?php if(form_error('ipoints')): ?>error<?php endif; ?>">
                                        <label>Add Recharge Points <span class="required">*</span></label>
                                        <input type="number" min="1" name="addRechargePoints" id="addRechargePoints" class="form-control required" value="<?php if(set_value('addRechargePoints')): echo set_value('addRechargePoints'); else: echo stripslashes($EDITDATA['addRechargePoints']);endif; ?>" placeholder="Add Recharge Points">
       ...
       var rechange_for = $('.rechange_for').val();
        ...
        if(rechange_for == 'Mobile No.') {
            ...
            if(parseInt(userData.slice(0,1)) == 0){
                ...
                var user = $(this).val();
                var ur      = '<?=getCurrentControllerPath('checkDeplicacy');?>';
                $.ajax({
                    url : ur,
                    method: "POST", 
                    data: {user: user},
        ...
        if(rechange_for == 'Email'){
            ...
            if(!regex.test(inputvalues)){    
                ...
                $.ajax({
                    ...
                    if(data1[1] != ""){
                        $('#availableReachargePoints').empty().append(data1[0]);
                        $('.recharge_btn').attr("disabled", false);
                    }else{
                        $('#availableReachargePoints').empty().append('Email id not exist.');
                        $('.recharge_btn').attr("disabled", true);
                    }
                    $('#userID').empty().val(data1[1]);
