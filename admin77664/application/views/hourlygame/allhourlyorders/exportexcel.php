...
<script>
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (responsesReceived == totalPage) {
                        ...
                        const filename = 'all-hourly-game-' + exportType + '-Data-' + curdate + '.xlsx';
                        const exportData = addSerialNumbers(allData);
                        downloadCSV(exportData, filename);

                        downloadLink.href = URL.createObjectURL(new Blob([convertToExcel(exportData)], { type: 'text/xlsx' }));
    ...
    function addSerialNumbers(data) {
        const rows = Array.isArray(data) ? data : JSON.parse(data);
        return rows.map((row, index) => ({ 'Sl.No': index + 1, ...row }));
    }
