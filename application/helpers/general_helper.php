...
if (!function_exists('ImageExist')) {
	function ImageExist($imageurl='') {
		 if (file_exists($imageurl) && exif_imagetype($imageurl) !== false):
		 	$imageurl  = base_url($imageurl);
		 else:
			$imageurl  = 'assets/img/NO_IMAGE.jpg';
		endif;
		return trim($imageurl);
	}
}
 ...
 * Encription
 */
if (!function_exists('dilipEncript')) {
	function dilipEncript($text) {
		$text	=	('DILIP').$text.('HALDER');
...
if (!function_exists('Encript')) {
	function Encript($text) {
		$text	=	('DILIP').base64_encode($text).('HALDER');
		return	base64_encode($text);
	}
}
...
if (!function_exists('Decript')) {
	function Decript($text) {
		...
		$text	=	str_replace(('DILIP'),'',$text);
		$text	=	str_replace(('HALDER'),'',$text);
 ...
 * Decription
 */
if (!function_exists('dilipDecript')) {
	function dilipDecript($text) {
		$text	=	base64_decode($text);
		$text	=	str_replace(('DILIP'),'',$text);
		$text	=	str_replace(('HALDER'),'',$text);
		return $text;
 ...
 * Encryption
 */
if (!function_exists('dilipEncrypt')) {
	function dilipEncrypt($text) {
		$text	=	('DILIP').$text.('HALDER');
		return	base64_encode($text);
	}
}
 ...
 * Decryption
 */
if (!function_exists('dilipDecrypt')) {
	function dilipDecrypt($text) {
		$text	=	base64_decode($text);
		$text	=	str_replace(('DILIP'),'',$text);
		$text	=	str_replace(('HALDER'),'',$text);
		return $text;
...
if (!function_exists('currentSystemId')) {
	function currentSystemId() {
		$currsystemid   	=  '';
		if($_COOKIE['CURRENT_SYSTEM_ID']){
			$currsystemid	=	$_COOKIE['CURRENT_SYSTEM_ID'];
		}else{			
			$currsystemid 	=	str_replace(".","",uniqid($_SERVER['REMOTE_ADDR'],true));
			setcookie('CURRENT_SYSTEM_ID',$currsystemid,time()+60*60*24*365,'/');
		}
		return $currsystemid;
	}
}
...
if (!function_exists('generateProductShareUrl')) {
	function generateProductShareUrl($products_id,$DZL_USERID,$DZL_USERS_REFERRAL_CODE) {
		...
		$userDetails = base64_encode($DZL_USERID.'_'.$DZL_USERS_REFERRAL_CODE.'_'.$date);
...
if (!function_exists('createStripePaymentIntent')) {
	function createStripePaymentIntent($inclusice_of_vat='',$user_email='',$order_id='') {	
